...
START_TEST(test_sscanf_1) {
  ...
  sscanf(input, format, &expected);
  s21_sscanf(input, format, &actual);

  ck_assert_int_eq(sscanf(input, format, &expected),
                   s21_sscanf(input, format, &actual));
...
START_TEST(test_sscanf_3) {
  const char *format = "%d %f %s %c";
  const char *input = "123 123.4 123 1";
  ...
  int actual_read = sscanf(input, format, &expected_int, &expected_float,
                           expected_str, &expected_char);
  int expected_read = s21_sscanf(input, format, &actual_int, &actual_float,
                                 actual_str, &actual_char);

  ck_assert_int_eq(actual_read, expected_read);
  ck_assert_int_eq(expected_int, actual_int);
  ck_assert_float_eq(expected_float, actual_float);
  ck_assert_str_eq(expected_str, actual_str);
}
END_TEST

START_TEST(test_sscanf_4) {
  const char *format = "%s %f %d %*c";
  const char *input = " 123 0001234.4 123 1   ";
  ...
  int expected_read = s21_sscanf(input, format, expected_str, &expected_float,
                                 &expected_int, &expected_char);
  int actual_read = sscanf(input, format, actual_str, &actual_float,
                           &actual_int, &actual_char);
...
START_TEST(test_sscanf_5) {
  const char *format = "%i %li %hi %ho %lu %x %X";
  const char *input = "99 0123 0xFF 000454 9999999 AA A0";

  int expected_int_i_10;
  long int expected_int_i_8;
  short int expected_int_i_16;
  unsigned short int expected_int_8;
  unsigned long int expected_int_10;
  unsigned int expected_int_16_1;
  unsigned int expected_int_16_2;

  int actual_int_i_10;
  long int actual_int_i_8;
  short int actual_int_i_16;
  unsigned short int actual_int_8;
  unsigned long int actual_int_10;
  unsigned int actual_int_16_1;
  unsigned int actual_int_16_2;

  int expected_read =
      s21_sscanf(input, format, &expected_int_i_10, &expected_int_i_8,
                 &expected_int_i_16, &expected_int_8, &expected_int_10,
                 &expected_int_16_1, &expected_int_16_2);
  int actual_read =
      sscanf(input, format, &actual_int_i_10, &actual_int_i_8, &actual_int_i_16,
             &actual_int_8, &actual_int_10, &actual_int_16_1, &actual_int_16_2);

  ck_assert_int_eq(actual_read, expected_read);
  ck_assert_int_eq(expected_int_i_10, actual_int_i_10);

  ck_assert_int_eq(expected_int_i_8, actual_int_i_8);
  ck_assert_int_eq(expected_int_i_16, actual_int_i_16);

  ck_assert_uint_eq(expected_int_8, actual_int_8);
  ck_assert_uint_eq(expected_int_10, actual_int_10);

  ck_assert_uint_eq(expected_int_16_1, actual_int_16_1);
  ck_assert_uint_eq(expected_int_16_2, actual_int_16_2);
}
END_TEST

START_TEST(test_sscanf_6) {
  const char *format = "%le %E %Lf %g %G";
  const char *input = "1.0e+1 123.4 110.1 1234.4 999.9";

  double expected_le;
  float expected_E;
  long double expected_Lf;
  float expected_g;
  float expected_G;

  double actual_le;
  float actual_E;
  long double actual_Lf;
  float actual_g;
  float actual_G;

  int expected_read = sscanf(input, format, &expected_le, &expected_E,
                             &expected_Lf, &expected_g, &expected_G);
  int actual_read = s21_sscanf(input, format, &actual_le, &actual_E, &actual_Lf,
                               &actual_g, &actual_G);

  ck_assert_int_eq(actual_read, expected_read);

  ck_assert_double_eq(expected_le, actual_le);
  ck_assert_float_eq(expected_E, actual_E);
  ck_assert_ldouble_eq(expected_Lf, actual_Lf);
  ck_assert_float_eq(expected_g, actual_g);
  ck_assert_float_eq(expected_G, actual_G);
}
END_TEST
...
START_TEST(test_sscanf_8) {
  ...
  sscanf(input, format, &expected_1, &expected_2);
  s21_sscanf(input, format, &actual_1, &actual_2);

  ck_assert_int_eq(sscanf(input, format, &expected_1, &expected_2),
                   s21_sscanf(input, format, &actual_1, &actual_2));
  ck_assert_int_eq(expected_1, actual_1);
  ck_assert_int_eq(expected_2, actual_2);
}
END_TEST

START_TEST(test_sscanf_9) {
  const char *format = "%3d %3d";
  const char *input = "000123456";
  int expected_1;
  int expected_2;
  int actual_1;
  int actual_2;
...
START_TEST(test_s21_trim_1) {
  ...
  char *result = s21_trim(src, s21_trim_chars);
...
START_TEST(test_s21_trim_4) {
  const char *src = "  Hello World  ";
  const char *s21_trim_chars = NULL;
  char *result = s21_trim(src, s21_trim_chars);
  ck_assert_str_eq(result, "  Hello World  ");
  free(result);
}
...
START_TEST(test_s21_insert_3) {
  char *result = s21_insert("hello ", "world", 6);
  ck_assert_str_eq(result, "hello world");
  free(result);
}
END_TEST
...
START_TEST(test_s21_insert_4) {
  char *result = s21_insert("test", "s21_inserted", -1);
  ck_assert_ptr_null(result);
  // No need to free result as it's NULL
}
END_TEST

START_TEST(test_s21_insert_5) {
  char *result = s21_insert("test", "s21_inserted", 5);
  ck_assert_ptr_null(result);
  // No need to free result as it's NULL
}
END_TEST
...
START_TEST(test_s21_insert_6) {
  char *result = s21_insert("", "content", 0);
  ck_assert_str_eq(result, "content");
  free(result);
}
END_TEST

START_TEST(test_s21_insert_7) {
  char *result = s21_insert("original", "", 3);
...
START_TEST(test_s21_insert_8) {
  char *result = s21_insert(NULL, "s21_inserted", 0);
  ck_assert_ptr_null(result);
  // No need to free result as it's NULL
}
END_TEST
...
START_TEST(test_s21_insert_9) {
  char *result = s21_insert("original", NULL, 0);
  ck_assert_ptr_null(result);
  // No need to free result as it's NULL
}
END_TEST
...
Suite *s21_sscanf_suite_1(void) {
  Suite *s = suite_create("S21_SSCANF_Suite");
